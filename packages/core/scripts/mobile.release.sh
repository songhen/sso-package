...
branch_prefix="core-mobile-sdk-release-v"
tag_prefix="CoreMobileSDK-v"

# Get release version from package.json
release_version=$(node -pe "require('./mobile-client/package.json').version")
...
git stash save -- $(git ls-files --exclude='./mobile-client/CHANGELOG.md') && done_emoji
...
rm -rf mobile-client/dist && done_emoji
...
# Create the src folder within the temp folder
mkdir -p $temp_folder/src

# Copy the necessary files to the temporary folder
cp -R packages/core/mobile-client/src/assets/images/* packages/core/mobile-client/dist/assets/images/
cp -R packages/core/mobile-client/src/assets/icons/* packages/core/mobile-client/dist/assets/icons/
cp -R packages/core/mobile-client/dist/ $temp_folder/src
cp -R packages/core/mobile-client/android $temp_folder/
cp -R packages/core/mobile-client/ios $temp_folder/
cp packages/core/mobile-client/wing-b2c-core-mobile-sdk.podspec $temp_folder/
cp packages/core/mobile-client/package.json $temp_folder/
cp packages/core/mobile-client/CHANGELOG.md $temp_folder/
done_emoji
